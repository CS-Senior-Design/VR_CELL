...
public class AnimationTest : MonoBehaviour
{
    ...
    // start positions
    private Vector3 _startPositionEnter = new Vector3(0.2f,1,-5);
    private Vector3 _startPositionExit = new Vector3(-0.1f,1,-5);
    // target positions
    private Vector3 _targetPositionEnter = new Vector3(-0.1f,1,-5);
    private Vector3 _targetPositionExit = new Vector3(-0.25f,1,-5);
    // used for the float coroutine
    private Vector3 _targetPositionFloat = new Vector3(-0.25f,1.5f,-5);
    ...
    void Update()
    {
        // use right click to trigger animation and only trigger the animation if neither of the animations are in progress
        if (Input.GetMouseButtonDown(1) && _enterInProgress == false && _exitInProgress == false)
        {
